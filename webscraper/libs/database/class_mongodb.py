...
class MonDatabase:
    ...
    def __init__(self, mongodb=None, database_name=None):
        if isinstance(mongodb,str):
            self._mongodb = MongoDB(conn_str=mongodb)
        else:
...
class MonCollection:
    ...
    :return: 无返回值
    """
    def __init__(self, mongodb=None, database=None, collection_name=None):
        if database is not None:
            if isinstance(database,MonDatabase):
                self._database = database
            elif isinstance(database,str):
                if mongodb is not None:
                    self._database = MonDatabase(mongodb=mongodb,database_name=database)
                else:
                    print('Please specify mongodb!')
                    raise Exception
            else:
